...
public class PullToRefreshAttacher implements View.OnTouchListener {
	...
	private final DecorChildLayout decorContents;
...
//		Log.d("TEST", " decorView.getHeight() = " + decorView.getHeight());
		...
		decorContents = new DecorChildLayout(activity, decorView, mHeaderView);

		if (decorView.getHeight() != 0) { // for some reason this case make headerView to be twice shifted to the top
			addPaddingForHeader();
		}
...
//		Log.d("TEST", " pullHeaderTopInset = " + pullHeaderTopInset);
		if (pullHeaderTopInset != AppData.UNDEFINED) {
			...
			decorContents.getHeaderViewWrapper().setPadding(0, statusBarHeight, 0, 0);
		}
	}

	public void removePaddingForHeader() {
		decorContents.getHeaderViewWrapper().setPadding(0, 0, 0, 0);
	}
...
//				return activity.getActionBar().getThemedContext(); // this requires explicit init of window that cause a lot of memory waste
	 ...
	 * {@link PullToRefreshAttacher}. Used with {@link #get(android.app.Activity, Options) get()}.
	 */
	...
	final static class DecorChildLayout extends FrameLayout {
		...
		public ViewGroup getHeaderViewWrapper() {
			return mHeaderViewWrapper;
		}
	}
