...
//	public long previousFGTime;
	...
	private ChatListenerImpl2 _chatListener;
	private ConnectionListenerImpl2 _connectionListener;
	private LccGameListener2 _gameListener;
	private LiveChessClient _lccClient;
	private User _user;
	private static LccHolder2 instance;

	private AndroidStuff2 androidStuff;
	...
	private final LccChallengeListener2 challengeListener;
	private final LccSeekListListener2 seekListListener;
	private final LccFriendStatusListener2 friendStatusListener;
	...
	private ChessClock2 whiteClock;
	private ChessClock2 blackClock;
...
//	private LccConnectionListener externalConnectionListener;
    ...
    public static LccHolder2 getInstance(Context context) {
        if (instance == null) {
            instance = new LccHolder2(context);

        }
        return instance;
    }

    private LccHolder2(Context context) {
		...
		_chatListener = new ChatListenerImpl2(this);
		_connectionListener = new ConnectionListenerImpl2(this);
		_gameListener = new LccGameListener2(this);
		challengeListener = new LccChallengeListener2(this);
		seekListListener = new LccSeekListListener2(this);
		friendStatusListener = new LccFriendStatusListener2(this);
		androidStuff = new AndroidStuff2(context);
...
//		liveChessClientEventListener.onAnotherLoginDetected();
	...
	public LccGameListener2 getGameListener() {
		return _gameListener;
	}

	public ChatListenerImpl2 getChatListener() {
		return _chatListener;
	}

	public ConnectionListenerImpl2 getConnectionListener() {
...
//		lccHolder.getAndroidStuff().closeLoggingInIndicator(); // TODO redundant
	...
	public AndroidStuff2 getAndroidStuff() {
		return androidStuff;
	}
	...
	public LccChallengeListener2 getChallengeListener() {
		return challengeListener;
	}

	public LccSeekListListener2 getSeekListListener() {
		return seekListListener;
	}

	public LccFriendStatusListener2 getFriendStatusListener() {
...
//		Log.d("TEST","game id = " + gameId);
	...
	public void makeMove(long gameId, final String move) {
		...
		try {
			...
			if (game.getSeq() >= 1) // we should start opponent's clock after at least 2-nd ply (seq == 1, or seq > 1)
			{
				...
				final ChessClock2 clockToBePaused = isWhiteRunning ? whiteClock : blackClock;
				final ChessClock2 clockToBeStarted = isWhiteRunning ? blackClock : whiteClock;
	...
	public ChessClock2 getBlackClock() {
		return blackClock;
	}

	public ChessClock2 getWhiteClock() {
		return whiteClock;
	}

	public void setWhiteClock(ChessClock2 whiteClock) {
		this.whiteClock = whiteClock;
	}

	public void setBlackClock(ChessClock2 blackClock) {
		this.blackClock = blackClock;
	}

	public void logout() {
		Log.d(TAG, "USER LOGOUT");
//		androidStuff.getContext().setLiveChess(false);
...
//	}
	...
	public void processFullGame() {
		...
		setWhiteClock(new ChessClock2(this, true, time));
		setBlackClock(new ChessClock2(this, false, time));
		// Do NOT call finish() outside of activity.
//		final Activity activity = getAndroidStuff().getLccEventListener();
//		if (activity != null) {
//			activity.finish();
//		}


		final Intent intent = new Intent(androidStuff.getContext(), GameLiveScreenActivity.class);
		...
		androidStuff.getContext().startActivity(intent);
	}
