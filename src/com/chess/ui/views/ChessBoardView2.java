...
public class ChessBoardView2 extends ImageView implements BoardViewFace {
...
//    private MainApp mainApp;
    ...
    public ChessBoardView2(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//
//        isHighlightEnabled = preferences.getBoolean(AppData.getUserName(this)
//                + AppConstants.PREF_BOARD_SQUARE_HIGHLIGHT, true);
//
//        showCoordinates = preferences.getBoolean(AppData.getUserName(this)
//                + AppConstants.PREF_BOARD_COORDINATES, true);
//
//
//        compStrength = preferences.getInt(AppData.getUserName(this)
//                + AppConstants.PREF_COMPUTER_STRENGTH, 0);
//
//        userName = AppData.getUserName(this);
    }

    private boolean need2ShowSubmitButtons() {
//        String sharedKey;
//        sharedKey = DataHolder.getInstance().isLiveChess() ? AppConstants.PREF_SHOW_SUBMIT_MOVE_LIVE : AppConstants.PREF_SHOW_SUBMIT_MOVE;
//        return preferences.getBoolean(AppData.getUserName(this)
//                + sharedKey, !DataHolder.getInstance().isLiveChess());
		return false;
    }
...
//        gameActivityFace.update(GameBaseActivity.CALLBACK_REPAINT_UI);    //movelist
        ...
        if (AppData.isLiveOrEchessGameMode(boardFace) && !boardFace.isAnalysis()) {
...
//        if (!AppData.isTacticsGameMode(boardFace) && isGameOver())
//            return;
...
//                    computerMove(mainApp.strength[preferences.getInt(AppData.getUserName(this) + AppConstants.PREF_COMPUTER_STRENGTH, 0)]);
//                    break;
//                }
...
//                saving += "|" + move.from + ":" + move.to + ":" + move.promote + ":" + move.bits;
//            }
//
...
//        } else if (boardFace.reps() == 3 && !DataHolder.getInstance().isLiveChess())
...
//				canvas.drawBitmap(mainApp.getBoardBitmap(), null, rect, null);
            }
        }
...
//                canvas.drawBitmap(mainApp.getPiecesBitmaps()[c][p], null, rect, null);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        track = false;
        if (!boardFace.isAnalysis()) {
            if (compmoving || AppData.isFinishedEchessGameMode(boardFace) || finished || boardFace.isSubmit() ||
                    (AppData.isLiveOrEchessGameMode(boardFace) && boardFace.getHply() < boardFace.getMovesCount()))
                return true;
...
//            if (AppData.isTacticsGameMode(boardFace) && boardFace.getHply() % 2 == 0) {
//                return true;
//            }
        }
...
//                        .getInt(AppData.getUserName(this)
//                                + AppConstants.PREF_COMPUTER_STRENGTH, 0)]);
//            }
            invalidate();
//            gameActivityFace.update(GameBaseActivity.CALLBACK_REPAINT_UI);
