 ...
 */
public class GameOnlineScreenActivity2 extends GameBaseActivity2 {
	...
	private class GameStateUpdateListener extends ChessUpdateListener2 {
	...
	public void invalidateGameScreen() {
		...
		boardView.addMove2Log(boardView.getBoardFace().getMoveListSAN()); // here we already have wrong moves list after analysis
	}
	...
	private class GetGameUpdateListener extends ChessUpdateListener2 {
	...
	private class SendMoveUpdateListener extends ChessUpdateListener2 {
			...
			if (returnedObj.contains(RestHelper.R_SUCCESS)) {
				...
				for (GameListItem currentGame : currentGames) {
					if (currentGame.getGameId() != gameId) {
						boardView.setBoardFace(new ChessBoard2(GameOnlineScreenActivity2.this));
						...
						// same new gameId
						Log.d("TEST", "getExtras().getLong previous game id = "
								+ getIntent().getExtras().getLong(GameListItem.GAME_ID));
						Log.d("TEST", "getLongExtra previous game id = "
								+ getIntent().getLongExtra(GameListItem.GAME_ID, 0));
	...
	private void checkMessages() {
		if (newGame.values.get(GameItem.HAS_NEW_MESSAGE).equals("1")) {
			...
			AppUtils.showNotification(this, StaticData.SYMBOL_EMPTY, gameId, StaticData.SYMBOL_EMPTY, StaticData.SYMBOL_EMPTY, ChatOnlineActivity.class);
		}
	}
	...
	private class AbortGameUpdateListener extends ChessUpdateListener2 {
	...
	private class DrawOfferedUpdateListener extends ChessUpdateListener2 {
