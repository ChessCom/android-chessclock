...
public class Online extends CoreActivity {
	...
	protected Dialog onCreateDialog(int id) {
		switch (id) {
			case 0: {
				if (temp_pos > -1) {
					final GameListElement el = mainApp.GameListItems.get(temp_pos);
					return new AlertDialog.Builder(this)
								...
								public void onClick(DialogInterface dialog, int whichButton) {
									if (appService != null) {
										appService.RunSingleTask(4,
												"http://www." + LccHolder.HOST + "/api/submit_echess_action?id=" + mainApp.sharedData.getString("user_token", "") + "&chessid=" + el.values.get("game_id") + "&command=ACCEPTDRAW&timestamp=" + el.values.get("timestamp"),
							...
							.setNeutralButton(getString(R.string.decline), new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog, int whichButton) {
									if (appService != null) {
										appService.RunSingleTask(4,
												"http://www." + LccHolder.HOST + "/api/submit_echess_action?id=" + mainApp.sharedData.getString("user_token", "") + "&chessid=" + el.values.get("game_id") + "&command=DECLINEDRAW&timestamp=" + el.values.get("timestamp"),
												null/*PD = MyProgressDialog.show(Online.this, null, getString(R.string.loading), true)*/
										);
									}
								}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		queries = new String[]{
				"http://www." + LccHolder.HOST + "/api/v2/get_echess_current_games?id=" + mainApp.sharedData.getString("user_token", "") + "&all=1",
				"http://www." + LccHolder.HOST + "/api/v2/echess_challenges?id=" + mainApp.sharedData.getString("user_token", ""),
				"http://www." + LccHolder.HOST + "/api/v2/get_echess_finished_games?id=" + mainApp.sharedData.getString("user_token", "")};
		...
		GamesType.post(new Runnable() {
			@Override
			public void run() {
				//GamesType.setSelection(mainApp.sharedData.getInt("gamestype", 1));
		...
		GamesType.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> a, View v, int pos, long id) {
				GamesAdapter = null;
				/*mainApp.SDeditor.putInt("gamestype", pos);
				mainApp.SDeditor.commit();*/
				currentListType = pos;
		...
		GamesList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View v, int pos, long id) {
				final GameListElement el = mainApp.GameListItems.get(pos);
				if (el.type == GameListElement.LIST_TYPE_CHALLENGES) {
					...
					if (mainApp.isLiveChess()) {
						...
						new AlertDialog.Builder(Online.this)
								...
								.setItems(new String[]{getString(R.string.accept), getString(R.string.decline)}, new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface d, int pos) {
										if (pos == 0) {
											String result = Web.Request("http://www." + LccHolder.HOST + "/api/echess_open_invites?id=" + mainApp.sharedData.getString("user_token", "") + "&acceptinviteid=" + el.values.get("game_id"), "GET", null, null);
											...
											String result = Web.Request("http://www." + LccHolder.HOST + "/api/echess_open_invites?id=" + mainApp.sharedData.getString("user_token", "") + "&declineinviteid=" + el.values.get("game_id"), "GET", null, null);
											if (result.contains("Success")) {
												Update(3);
											} else if (result.contains("Error+")) {
												mainApp.ShowDialog(Online.this, "Error", result.split("[+]")[1]);
					...
					mainApp.SDeditor.putString("opponent", el.values.get("opponent_username"));
					mainApp.SDeditor.commit();
							...
							.setItems(new String[]{getString(R.string.chat), getString(R.string.drawoffer), getString(R.string.resignorabort)}, new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface d, int pos) {
									if (pos == 0) {
										...
										if (el.values.get("is_draw_offer_pending").equals("p"))
											Draw = "ACCEPTDRAW";
										String result = Web.Request("http://www." + LccHolder.HOST + "/api/submit_echess_action?id=" + mainApp.sharedData.getString("user_token", "") + "&chessid=" + el.values.get("game_id") + "&command=" + Draw + "&timestamp=" + el.values.get("timestamp"), "GET", null, null);
										if (result.contains("Success")) {
										...
										String result = Web.Request("http://www." + LccHolder.HOST + "/api/submit_echess_action?id=" + mainApp.sharedData.getString("user_token", "") + "&chessid=" + el.values.get("game_id") + "&command=RESIGN&timestamp=" + el.values.get("timestamp"), "GET", null, null);
										if (result.contains("Success")) {
											Update(1);
										} else if (result.contains("Error+")) {
											mainApp.ShowDialog(Online.this, "Error", result.split("[+]")[1]);
		...
		findViewById(R.id.tournaments).setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("http://www." + LccHolder.HOST + "/login.html?als=" + mainApp.sharedData.getString("user_token", "") + "&goto=" + GOTO)));
			}
		});
		findViewById(R.id.stats).setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				String GOTO = "http://www." + LccHolder.HOST + "/echess/mobile-stats/" + mainApp.sharedData.getString("username", "");
				try {
					GOTO = URLEncoder.encode(GOTO, "UTF-8");
				} catch (UnsupportedEncodingException e) {
				}
		...
		currentGame.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				/*}
							...
							{
							  e.printStackTrace();
							  System.out.println("!!!!!!!! mainApp.gameId " + mainApp.gameId);
							  System.out.println("!!!!!!!! lccHolder.getGame(mainApp.gameId) " + lccHolder.getGame(mainApp.gameId));
							}*/
			}
		});
		if (mainApp.isLiveChess()) {
				...
				public View getView(int position, View convertView, ViewGroup parent) {
					...
					button.setOnClickListener(new OnClickListener() {
						public void onClick(View view) {
							mainApp.SDeditor.putString("initialTime", "" + startNewGameButton.getMin());
							mainApp.SDeditor.putString("bonusTime", "" + startNewGameButton.getSec());
							mainApp.SDeditor.commit();
	...
	public void LoadPrev(int code) {
		//finish();
		mainApp.mTabHost.setCurrentTab(0);
	}
	...
	public void Update(int code) {
		if (code == -1) {
			if (appService != null) {
				if (!mainApp.isLiveChess()) {
					appService.RunRepeatbleTask(ONLINE_CALLBACK_CODE, 0, UPDATE_DELAY,
							queries[/*mainApp.sharedData.getInt("gamestype", 1)*/currentListType],
			...
			//GamesList.setVisibility(View.GONE);
			mainApp.GameListItems.addAll(tmp);
			if (GamesAdapter != null) {
				GamesAdapter.notifyDataSetChanged();
			}
			GamesList.setVisibility(View.VISIBLE);
			if (GamesAdapter == null) {
				GamesAdapter = new OnlineGamesAdapter(Online.this, R.layout.gamelistelement, mainApp.GameListItems);
