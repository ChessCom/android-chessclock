...
public class HomePlayFragment extends CommonLogicFragment implements SlidingMenu.OnOpenedListener{
	...
	private void loadRecentOpponents() {
		Cursor cursor = DbDataManager1.getRecentOpponentsCursor(getActivity(), getUsername());// TODO load avatars
		if (cursor != null && cursor.moveToFirst()) {
			if (cursor.getCount() >= 2) {
				...
				firstFriendUserName = DbDataManager1.getString(cursor, DbScheme.V_WHITE_USERNAME);
				friendUserName1Txt.setText(firstFriendUserName);
				friendRealName1Txt.setText(DbDataManager1.getString(cursor, DbScheme.V_WHITE_USERNAME));
				cursor.moveToNext();
				secondFriendUserName = DbDataManager1.getString(cursor, DbScheme.V_WHITE_USERNAME);
				friendUserName2Txt.setText(secondFriendUserName);
				friendRealName2Txt.setText(DbDataManager1.getString(cursor, DbScheme.V_WHITE_USERNAME));
	...
	private void setRatings() {
		...
		int liveRating = DbDataManager1.getUserCurrentRating(getActivity(), DbScheme.Tables.GAME_STATS_LIVE_STANDARD.ordinal(), getUsername());
		liveRatingTxt.setText(String.valueOf(liveRating));

		// set daily rating
		int dailyRating = DbDataManager1.getUserCurrentRating(getActivity(), DbScheme.Tables.GAME_STATS_DAILY_CHESS.ordinal(), getUsername());
