...
public class BoardView extends ImageView {
	...
	public boolean compmoving = false;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				try {
					...
					Log.d("BoardView", "mainApp.boardBitmap " + mainApp.getBoardBitmap());
					return;
				}
			}
		}
		...
		if (!compmoving && !MainApp.isTacticsGameMode(this))
			if (W < H) {
				...
				// Count for existing piecesBitmap. If piece exist on boardBitmap, decrease it counter so it will not be redrawn in old version
				for (i = 0; i < 64; i++) {
					// count for existing piecesBitmap. If piece exist on bo
