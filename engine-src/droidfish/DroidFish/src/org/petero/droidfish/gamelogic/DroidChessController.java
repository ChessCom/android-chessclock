...
public class DroidChessController {
    ...
    /** Start a new game. */
    public final synchronized void newGame(GameMode gameMode, TimeControlData tcData, int depth) {
        ...
        searchId++;
        game = new Game(gameTextListener, tcData, depth);
    ...
    public final synchronized void setFENOrPGN(String fenPgn) throws ChessParseError {
        Game newGame = new Game(gameTextListener, game.timeController.tcData, game.getDepth());
			try {
    ...
    private void updateComputeThreads() {
        ...
        listener.clearSearchInfo(searchId);
        updateBookHints();
		//Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads 2");
        if (!computerPlayer.sameSearchId(searchId)) {
			/*Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads 3");
			Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads computersTurn " + computersTurn);
			Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads ponder " + ponder);*/
            if (analysis) {
				...
				//Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads computersTurn || ponder " + (computersTurn || ponder));
                listener.clearSearchInfo(searchId);
				...
				int moveTime = game.timeController.getEngineMaxTimeLimit(true);
                ...
                SearchRequest sr = DroidComputerPlayer.SearchRequest.searchRequest(
                        ...
                        strength, moveTime, depth);
                computerPlayer.queueSearchRequest(sr);
            } else {
				//Log.d(CompEngineHelper.TAG, "@@@@@@@@ updateComputeThreads 4 queueStartEngine");
                computerPlayer.queueStartEngine(searchId, engine);
            }
        }
    }
        ...
        for (Move m : moves) {
            if ((m.from == move.from) && (m.to == move.to)) {
                ...
                if (m.promoteTo == promoteTo) {
					...
					Log.d(MyFish.TAG, "Controller doMove = " + strMove);
					return true;
				}
            }
        }
