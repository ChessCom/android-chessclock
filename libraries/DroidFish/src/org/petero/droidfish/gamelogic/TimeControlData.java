...
public final class TimeControlData {
    ...
    public static final class TimeControlField {
		...
		int engineMaxTimeLimit;

        public TimeControlField(int time, int moves, int inc, int engineMaxTimeLimit) {
            timeControl = time;
            movesPerSession = moves;
            increment = inc;
			this.engineMaxTimeLimit = engineMaxTimeLimit;
        }
    }
    ...
    public TimeControlData() {
        ...
        tcW.add(new TimeControlField(5*60*1000, 60, 0, 5000));
        tcB = new ArrayList<TimeControlField>();
        tcB.add(new TimeControlField(5*60*1000, 60, 0, 5000));
    }

    /** Set a single time control for both white and black. */
    public final void setTimeControl(int time, int moves, int inc, int engineMaxTimeLimit) {
        tcW = new ArrayList<TimeControlField>();
        tcW.add(new TimeControlField(time, moves, inc, engineMaxTimeLimit));
        tcB = new ArrayList<TimeControlField>();
        tcB.add(new TimeControlField(time, moves, inc, engineMaxTimeLimit));
    }
    ...
    private static boolean arrayEquals(ArrayList<TimeControlField> a1,
        ...
        for (int i = 0; i < a1.size(); i++) {
            ...
            if ((f1.timeControl != f2.timeControl) ||
				...
				(f1.engineMaxTimeLimit != f2.engineMaxTimeLimit))
                return false;
        }
        return true;
    }
    ...
    public void readFromStream(DataInputStream dis, int version) throws IOException {
        for (int c = 0; c < 2; c++) {
            ...
            for (int i = 0; i < nw; i++) {
                ...
                int inc = dis.readInt();
				int engineMaxTimeLimit = dis.readInt();
                tc.add(new TimeControlField(time, moves, inc, engineMaxTimeLimit));
            }
        }
    }
    ...
    public void writeToStream(DataOutputStream dos) throws IOException {
        for (int c = 0; c < 2; c++) {
            ...
            for (int i = 0; i < nw; i++) {
				...
				dos.writeInt(tcf.engineMaxTimeLimit);
            }
        }
    }
}
